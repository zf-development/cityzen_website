<template>
  ...
  const {
    ...
    title?: string;
    icon?: string;
    type?: 'default' | 'info' | 'warning' | 'success' | 'danger' | 'fdanger' | 'secondary';
    to?: string;
    target?: Target;
    external?: boolean;
    inStack?: boolean;
  ...
  const typeTwClass = {
    ...
    finfo: 'border-sky-600 text-white bg-sky-600 [&>svg]:text-white',
    warning: 'border-amber-600 text-amber-600 [&>svg]:text-amber-600',
    fwarning: 'border-amber-600 text-white bg-amber-600 [&>svg]:text-white',
    success: 'border-green-600 text-green-600 [&>svg]:text-green-600',
    fsuccess: 'border-green-600 text-white bg-green-600 [&>svg]:text-white',
    danger: 'border-red-600 text-red-600 [&>svg]:text-red-600',
    fdanger: 'border-red-600 text-white bg-red-600 [&>svg]:text-white',
